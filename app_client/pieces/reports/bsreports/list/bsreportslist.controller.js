...
angular
	.module('ssacpos')
	.controller('bsreportslistCtrl', bsreportslistCtrl);

bsreportslistCtrl.$inject = ['$window','$location','$scope','$compile','bsreport','exportservice'];

function bsreportslistCtrl($window,$location,$scope,$compile,bsreport,exportservice) {
	var vm = this;
	...
	vm.createBsReport = function() {
		...
		var stringToAppend = "<div class='col-xs-12'><bsreportcreate></bsreportcreate></div>";
	...
	vm.editBsReport = function(bsreportid){
		...
		$(".bsreports-container").empty();
		var stringToAppend = "<div class='col-xs-12'><bsreportsedit></bsreportsedit></div>";
		var el = angular.element(stringToAppend)
		$(".bsreports-container").append(el);
		compiled = $compile(el);
		compiled($scope);
	...
	vm.showList = function(){
		$(".dialogbox").hide();
		$(".bsreports-container").empty();
		var stringToAppend = "<div class='col-xs-12'><bsreports></bsreports></div>";
		var el = angular.element(stringToAppend)
		$(".bsreports-container").append(el);
		compiled = $compile(el);
		compiled($scope);
	}
	vm.exportBsReports = function(){
		exportservice.exportCSV(vm.bsreports,"BsReports").then(function(response){
			console.log(response);
			$(".dialogbox").empty();
			var appendString = "<div class='row'>"
							 +  "<div class='col-xs-12'>"
