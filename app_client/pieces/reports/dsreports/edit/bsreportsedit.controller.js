...
angular
	.module('ssacpos')
	.controller('bsreportsEditCtrl', bsreportsEditCtrl);

bsreportsEditCtrl.$inject = ['$window','$location','$scope','$compile','bsreport','authentication','user','invoice'];

function bsreportsEditCtrl($window,$location,$scope,$compile,bsreport,authentication,user,invoice) {
	var vm = this;
	vm.isLoggedIn = authentication.isLoggedIn();
	if(vm.isLoggedIn){
		vm.currentBsReport = bsreport.getBsReport();
		user.getUsers().then(function(response){
			vm.employees = response.data;
			invoice.getInvoiceList().then(function(response){
				vm.invoices = response.data;
			})
			bsreport.getBsReportById(vm.currentBsReport.id).then(function(response){
				vm.currentBsReport = response.data;
				console.log(vm.currentBsReport);
				$("#report-startdate").val(moment(vm.currentBsReport.startdate).format("MM/DD/YYYY"));
				$("#report-enddate").val(moment(vm.currentBsReport.enddate).format("MM/DD/YYYY"));
				$("#report-employee").val(vm.currentBsReport.employee);
			})
		})
		vm.submitBsReport = function(){
			vm.bsreport = {
				_id : vm.currentBsReport._id,
			...
			console.log(vm.bsreport);
			bsreport.editBsReport(vm.bsreport).then(function(response){
