...
function tpreportsCtrl($window,$location,$scope,$compile,tpreport) {
	...
	vm.createTpReport = function() {
		...
		var stringToAppend = "<div class='col-xs-12 piece'><tpreportcreate></tpreportcreate></div>";
	...
	vm.editTpReport = function(tpreportid){
		...
		$(".data-container").empty();
		var stringToAppend = "<div class='col-xs-12 piece'><tpreportsedit></tpreportsedit></div>";
		var el = angular.element(stringToAppend)
		$(".data-container").append(el);
		compiled = $compile(el);
		compiled($scope);
